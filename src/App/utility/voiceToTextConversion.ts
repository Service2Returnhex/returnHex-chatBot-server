...
export const readAudioNGenerateText = async (filePath: string): Promise<string> => {
  ...
  try {
    ...
    return "Sorry, I couldn't process the audio message.";
      ...
      console.error('Error in voice to text conversion:', error?.response?.data || error.message || error);
      return "Sorry, I couldn't process the audio message.";  
  } finally {
    stream.destroy?.();
    fs.unlink(filePath, () => {});
