...
export const makePromtDM = async (
  ...
  let postList = "";
  const recentPost = posts.length > botConfig.postVisibility 
  ? posts.slice(-botConfig.postVisibility) : posts
  ...
  ${i + 1}. ${!p.summarizedMsg ? p.message : p.summarizedMsg}`
    ...
    postList +=  `\n visite our page for ${posts.length - recentPost.length} more(can be products, service etc page category)...` : '' 
  }
