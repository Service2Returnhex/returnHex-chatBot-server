...
const createAndTrainProduct = async (payload: IPost) => {
  // ensure DB connected
  ...
  console.log(
    "TRAIN_PRODUCT incoming payload:",
  ...
  const needsEnrichment = images.some(
    (img) =>
      !Array.isArray(img.embedding) || img.embedding.length === 0 || !img.phash
  );
  ...
  if (needsEnrichment) {
    try {
      enrichedImages = await sanitizeAndEnrichImages(
        images,
      ...
      console.warn(
        "Image enrichment failed (will continue without embeddings):",
        e
      );
  ...
  const embeddingsList = enrichedImages
